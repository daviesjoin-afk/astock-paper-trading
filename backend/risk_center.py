...
# 行情来源白名单。只认 == "live" 会把生产里真实在用的 eastmoney-clist /
# tencent_public_quote 等来源当成"没有实时行情"，风控面板据此误判行情覆盖率。
LIVE_QUOTE_SOURCES = frozenset({
    "live",
    "dashboard_cache",
    "live_snapshot",
    "eastmoney-clist",
...
def refresh_snapshot(*, market, positions, universe, snapshot_at, news_events, news_error, sector_rows):
    ...
    quote_coverage = (
        sum(position.get("quote_source") in LIVE_QUOTE_SOURCES for position in positions)
        / len(positions) * 100
    ...
    live_positions = [
        ...
        if position.get("quote_source") in LIVE_QUOTE_SOURCES
    ]
...
def build_dashboard(base_dashboard, snapshot):
    ...
    worst = _worst_level([card["level"] for card in cards] + [
        # 只有数据源**严重失败**（failed）或动态风控熔断（halt）才禁止开仓；
        # 数据源未就绪（unknown）或过期（stale）降级为**收紧额度**（tightened），
        # 不把"暂时没拿到行情"误封成禁止开仓。
        "blocked" if any(
    ...
    overall_summary = {
        "normal": "当前参与策略未触发新开仓熔断，新增因子仍处于影子观察。",
    ...
    return {
        ...
        "overall": {
            ...
            "trade_permission": (
                ...
                if worst == "blocked" else "当前参与策略共用同一资金池；按各自单票、行业和止损模型执行"
            ),
