...
#: 调用方必须提供的业务列；provenance 四列由 context 独占供给。
_ROW_COLUMNS = (
    "signal_date",
    "intended_date",
    "code",
...
def commit_signal(
    ...
    *,
    ...
    ``context`` 是该批次冻结的写入上下文：provenance 四列**只**从这里取，
    ``row`` 只提供业务列。这使「候选批用 A 战略版本构建、落库却盖 B 版本戳」
    在类型层面不可表达。
    ...
    context:
        R23 resolver 产出的 frozen :class:`SignalWriteContext`。
    row:
        业务列映射，必须包含 :data:`_ROW_COLUMNS` 全部键。
    conflict:
