...
def _recent_non_weekend_day(day: dt.date) -> dt.date:
    """最近的一个"星期几不是周末"的日期（含当天）。

    名字刻意不叫 "trading day"：它**不是**交易日历。本仓库的完整交易日历（节假日、半日市）
    属于 ``tradability_archive`` / 执行层的口径，这里只需要一个"不是周六周日"的业务日，
    好让 ``execution_planner._session_phase`` 落在连续竞价时段，从而让 fixture 不依赖
    "CI 跑在星期几"。
    """
    candidate = day
...
class IntradaySellStampTests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # 前提。这里只把日期对齐到最近的**非周末日**，从而消除这一维度的不确定性。
        # 执行侧的 session 规则一个字都没改（本修复只动测试的日期取值）。
        self.today = _recent_non_weekend_day(dt.date.today())
        self.code = "600001"
