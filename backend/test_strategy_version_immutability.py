...
修复后的规则由数据库与 domain 共同表达：

- 数据库触发器只允许删除 ``origin='user' AND lifecycle_status='draft'``
  定义名下的 version 行——正式版本（validated/active/paused/retiring/
  archived，以及 builtin）在任何时刻都不可删除；
