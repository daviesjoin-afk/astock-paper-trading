...
export async function api(path, options){
  ...
  // 即跳出，正常请求不受影响。
  // PR-2 复审：重试**只对只读方法**开放。此前该上限与 method 无关，若调用方
  // 传 options.method='POST'（本函数是通用入口），一次 502/503/504 就会被自动
  // 重放最多 4 次，与"写操作不重试、避免重复副作用"的原则冲突。
  // 现在 mutation 永远只尝试 1 次。
  var MAX_READ_ATTEMPTS = 4;
  var method=String(options.method||'GET').toUpperCase();
  var attempts=READ_METHODS.indexOf(method)>=0?MAX_READ_ATTEMPTS:1;
  for(var attempt=0; attempt<attempts; attempt++){
    ...
    catch(err){ if(attempt===attempts-1) throw err; await new Promise(function(resolve){setTimeout(resolve,800*(attempt+1));}); continue; }
    finally { if(timeout) clearTimeout(timeout); }
    if(r.status!==502 && r.status!==503 && r.status!==504) break;
    if(attempt<attempts-1) await new Promise(function(resolve){setTimeout(resolve,800*(attempt+1));});
  }
