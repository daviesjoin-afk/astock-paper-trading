...
test.describe("策略深链接路由", () => {
  ...
  test("B. 站内跳转与直接加载等价，且刷新后仍是同一条详情", async ({ page }) => {
    const id = uniqueId(`${PREFIX}_nav`);
    await openWorkbench(page);
    await createDraftViaUi(page, { id, name: "E2E 站内跳转" });

    // 从纸盘页经"在策略工坊打开"回到工作坊（真实按钮）
    await page.getByTestId("main-nav-paper").click();
    await page.getByTestId("paper-module-tabs").getByText("运行策略").click();
    await page.getByTestId(`paper-open-workbench-${id}`).click();
