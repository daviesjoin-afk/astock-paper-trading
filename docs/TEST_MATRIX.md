...
| 行读取不假设 `row_factory`（`cursor.description` + `zip` 自组装 dict，裸连接亦可用） | `paper_cycle_ownership._rows` | `test_paper_cycle_ownership.py`（`test_row_reader_does_not_assume_the_connection_row_factory`） | ✅ |
...
| 负向变异验证（N1–N12 必须让守卫变红：N1 账本所有权错误剔除 paused、N2 执行参与者不再剔除 paused、N3 执行参与者改用注册表 active 作用域、N4 显式空集错误回落内置五套、N5 忽略 `paper_accounts.cycle_id` 绑定、N6 删除 `cycle_not_configured` 回退、N7 删除未挂接周期回退、N8 参与者顺序漂移、N9 把解析器复制回 `paper_trading`、N10 解析器反向 import `paper_trading`、N11 解析器引入写操作、N12 风控退出被收窄为执行参与者 ⇒ 合计 12/12） | — | 手工执行 N1–N12 变异脚本（源码级变异逐字节备份 + `finally` 还原 + `git hash-object` 校验；见 PR 描述） | ✅ |
