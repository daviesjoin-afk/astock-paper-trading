...
内置模板之外，可以在看板的 **策略工坊** 里定义自己的声明式策略（不写 Python）：

1. **新建草稿**：填策略 ID（`^[a-z][a-z0-9_]{2,63}$`，创建后不可改）与名称；用可视化条件或直接写 DSL。
2. **预览**：调用编译管线，展示系统硬边界 / 策略风控 / 可进化参数三段；预览不写库。
3. **保存草稿**：写入定义与第一个不可变版本（含 structure checksum）。
4. **验证 → 激活**：激活要求运行时就绪（DSL 可编译、风险指纹、风险画像、执行画像全部可编译）。用户策略默认从**试点**资金阶段（25%）起步。
5. **加入下一周期**：到 **设置中心 → 模拟盘与资金 → 启用策略** 勾选它；保存后只影响**下一**周期，当前周期快照不变。全部取消 = Idle 周期（合法）。
6. **暂停 / 恢复 / 退役 / 归档**：暂停只移除执行资格，当前周期里已归属的资金与净值仍留在账本；退役后走归档。
7. **删除限制**：只有**从未离开草稿生命周期且没有任何历史引用**的用户草稿可以物理删除；其余只能归档（页面对此有提示）。

模型与不变量见 [`STRATEGY_PLATFORM.md`](STRATEGY_PLATFORM.md)。也可以先克隆一个内置模板再改。
