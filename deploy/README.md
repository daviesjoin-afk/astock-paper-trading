...
| `docker-compose-server` | `<SERVER_PATH>`，`astock-codex`/worker 容器 | `/etc/cron.d/astock-codex` | 3 分钟 | `docker-compose.server.yml` + `astock-codex.cron` |
...
```bash
rsync -av --exclude-from=deploy/<REDACTED_SSH_TARGET>:/tmp/astock-quant/
```
