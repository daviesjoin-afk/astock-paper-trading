...
for DB in paper_trading.sqlite3 adaptive_learning.sqlite3 paper_research.sqlite3 selection_tracking.db; do
  if [[ -f <SERVER_PATH>$DB ]]; then
    sqlite3 "<SERVER_PATH>$DB" ".backup '$DEST/$DB'"
    echo "backup $DB ok"
  fi
done

# 2. 关键配置与清单
cp <SERVER_PATH>kline_manifest.json "$DEST/" 2>/dev/null || true
cp <SERVER_PATH>universe.json "$DEST/" 2>/dev/null || true
cp /etc/nginx/conf.d/astock-codex.conf "$DEST/astock-codex.nginx.conf" 2>/dev/null || true
cp <SERVER_PATH> "$DEST/" 2>/dev/null || true
cp /etc/cron.d/astock-codex "$DEST/astock-codex.cron" 2>/dev/null || true
cp <SERVER_PATH> "$DEST/astock-codex.cron.source" 2>/dev/null || true
...
# 4. 清理过期备份
find <SERVER_PATH> -mindepth 1 -maxdepth 1 -type d -mtime +"$KEEP_DAYS" -exec rm -rf {} + 2>/dev/null || true
