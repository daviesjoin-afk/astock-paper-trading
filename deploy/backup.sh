...
for DB in paper_trading.sqlite3 adaptive_learning.sqlite3 paper_research.sqlite3 selection_tracking.db; do
  if [[ -f /root/codex/data_cache/$DB ]]; then
    sqlite3 "/root/codex/data_cache/$DB" ".backup '$DEST/$DB'"
    echo "backup $DB ok"
  fi
done

# 2. 关键配置与清单
cp /root/codex/data_cache/kline_manifest.json "$DEST/" 2>/dev/null || true
cp /root/codex/data_cache/universe.json "$DEST/" 2>/dev/null || true
cp /etc/nginx/conf.d/astock-codex.conf "$DEST/astock-codex.nginx.conf" 2>/dev/null || true
cp /root/codex/docker-compose.server.yml "$DEST/" 2>/dev/null || true
cp /etc/cron.d/astock-codex "$DEST/astock-codex.cron" 2>/dev/null || true
cp /root/codex/deploy/astock-codex.cron "$DEST/astock-codex.cron.source" 2>/dev/null || true
...
# 4. 清理过期备份
find /root/backups/daily -mindepth 1 -maxdepth 1 -type d -mtime +"$KEEP_DAYS" -exec rm -rf {} + 2>/dev/null || true
