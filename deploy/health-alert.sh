...
DB_SIZE=$(stat -c %s <SERVER_PATH>paper_trading.sqlite3 2>/dev/null || echo 0)
...
# 不会运行。
RECENT_ERR=$(tail -400 <SERVER_PATH> 2>/dev/null | grep -c '"error"')
RECENT_BLOCKED=$(tail -400 <SERVER_PATH> 2>/dev/null | grep -c 'blocked')
