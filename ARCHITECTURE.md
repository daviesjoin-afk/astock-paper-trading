...
本次先同步服务器工作区并修复三个可验证边界：前端缓存版本/运行时镜像一致性、活动页风控审计请求上限，以及风险 outbox 在纸盘已提交后的 adaptive 侧重放收敛。随后新增 `decision_context.py` 集中证据读取、`decision_rules.py` 承载纯规则、`marketdata_transport.py` 承载 HTTP 传输、`paper_trading_rules.py` 承载交易日/费用/证券权限规则、`paper_storage.py` 隔离 SQLite 连接生命周期，并以 `paper_portfolio.py` 承载持仓 lot 聚合；兼容导出和回归测试守住现有行为。更大范围的迁移和模块拆分仍按 `docs/PRD-architecture-hardening.md` 分阶段推进。
