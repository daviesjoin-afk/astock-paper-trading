...
- `backend/marketdata_providers.py` 负责腾讯/新浪实时行情文本解析；网络请求、重试和源切换仍由 `data_fetcher.py` 编排，解析器本身无 I/O。
