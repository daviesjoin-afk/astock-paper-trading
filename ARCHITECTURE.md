...
| 流动性与部分成交 | 可成交数量受行情 amount / price × 1% participation cap 约束并按 lot 取整；未成交部分保留在 `remaining_qty`，下次使用新的 quote 重试 |
...
| 幂等与并发 | fill event key 固定绑定 order + quote observation + ruleset；数据库唯一键、执行版本 CAS 与账务 writer transaction 共同阻止重复成交、超额成交和资金/持仓不一致 |
| lineage | 执行只消费持久订单上的 signal、cycle、strategy version/checksum；cycle 守卫验证订单和账户归属，不用新策略版本改写历史委托 |

执行证据投影持久化 `execution_asof`、市场 freshness / verification / as-of、tradability
facts、reason codes、pricing basis、slippage、fees 与 ruleset version。前端仅按后端结果
展示工作中、部分成交、已成交、拒绝、撤销，以及逐笔成交数量和证据；T+1、时段、封板、
...
- 回归门禁：`backend/test_execution_planner.py`（规则、状态、费用、滑点与委托 writer），
  ...
  `backend/test_sell_fill_commit_convergence.py`；前端展示由
  ...
  cancel、historical as-of、duplicate event、fees、slippage 和 locked limit，必须全部 CAUGHT。
