...
仓库完整保留五套策略定义，并统一纳入策略注册表、自适应研究、历史回放和审计展示。当前新周期的正式模拟账户仍由两套活动策略驱动；其余三套策略保留为 legacy 研究/回放口径，不会被误分配新周期资金或调度时间。

| 策略 | 状态 | 风格 | 主要用途 |
|---|---|---|---|
| `tq_breakout` | active | 强势突破 | 放量、资金共振后的短周期突破候选 |
| `trend_pullback` | legacy | 趋势回调 | 中期上行趋势中的缩量回调观察与历史回放 |
| `sector_rotation` | legacy | 板块轮动 | 热点板块排名、资金共振与个股相对强度轮动研究 |
| `reported_profit_breakout` | legacy | 质量突破 | 财报/预告驱动的业绩突破评分与历史回放 |
| `main_force_top10` | active | 主力资金 | 跟踪主力净流入靠前、通过实时确认的候选 |

两套 active 策略共用撮合、共享资金池、风控与审计基础设施，但拥有独立的候选车道、仓位席位和退出规则；五套定义都不会被自动删除、重命名或隐式替换。
