...
def main():
    ...
    if marker not in text:
        strategies.write_text(text + STRATEGIES_APPEND, encoding="utf-8")
    ...
    if old in text:
        if text.count(old) != 1:
            raise RuntimeError("paper selection whitelist count changed")
        paper.write_text(text.replace(old, new, 1), encoding="utf-8")
    elif new not in text:
        raise RuntimeError("paper selection whitelist anchor missing")
    ...
    if marker not in text:
        ...
        if anchor not in text:
            raise RuntimeError("test module anchor not found")
        tests.write_text(text.replace(anchor, "\n" + TEST_APPEND + anchor, 1), encoding="utf-8")
