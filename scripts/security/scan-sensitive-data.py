...
def scan_text(text: str, location: str, findings: Findings, *, include_long_hex=True, allow: "AllowList | None" = None):
    ...
    def skip(value: str) -> bool:
        return bool(allow) and (allow.evaluate_value(value) or allow.evaluate_location(location))
        ...
        for match in pattern.finditer(text):
            ...
            if kind == "PRIVATE_EMAIL":
                ...
                if candidate.startswith(("[", "<")) and candidate.endswith(("]", ">")):
                    continue
                # Code identifiers/references: `token = _get_token(`, `token = data['x']`
                if kind == "SECRET_ASSIGN" and (
...
def scan_worktree(repo: str, findings: Findings, manual_review: list, allow: "AllowList"):
    for path, rel in iter_worktree_files(repo):
        ...
        if allow.evaluate_location(f"worktree:{rel}") or SCANNER_SELF_RE.search(rel):
            continue
        if SENSITIVE_FILE_RE.search(rel) and not rel.endswith(SENSITIVE_FILE_ALLOW) and not FIXTURE_PATH_RE.search(rel):
...
def scan_history(repo: str, findings: Findings, manual_review: list, allow: "AllowList"):
    ...
    for record in raw.split("\x1e"):
        ...
        for name_field, name_value in (("author-name", an), ("committer-name", cn)):
            low = name_value.lower()
            if any(marker in low for marker in ("jiamianh", "daviesjoin")) and "users.noreply" not in low:
