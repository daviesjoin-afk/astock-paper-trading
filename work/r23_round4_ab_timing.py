...
每次测量都是独立子进程 + 独立临时数据目录，环境完全一致；脚本会**断言**实际调度
真的是交错的（每一轮都包含两个版本、每轮首个版本在轮间交替、且同版本的最长连续段
不超过 2），并把真实顺序打印出来供审核。

注意 AB/BA 轮换在翻转处必然出现一次相邻的同版本（``… master,R23 | R23,master …``），
那是正常形状而非块状测量，所以判定用的是「最长连续段」而不是「相邻必不同」。
